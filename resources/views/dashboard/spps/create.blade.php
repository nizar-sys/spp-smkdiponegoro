...
@section('title-header', 'Tambah Data SPP')
@section('breadcrumb')
        ...
        <div class="col-12">
            <div class="card shadow">
                <div class="card-header bg-transparent border-0 text-dark">
                    <h5 class="mb-0">Formulir Tambah Data SPP</h5>
                </div>
                <div class="card-body">
                    <form action="{{ route('data-spp.store') }}" method="POST" role="form"
                                    ...
                                    <select class="form-control @error('siswa_id') is-invalid @enderror" id="siswa_id"
                                        ...
                                        @foreach ($students as $student)
                                            <option value="{{ $student->id }}"
                                                @if (old('siswa_id') == $student->id) selected @endif>
                            ...
                            <div class="col-12">
                                <div class="form-group mb-3">
                                    ...
                                    <input type="number" class="form-control @error('nominal') is-invalid @enderror"
                                        id="nominal" placeholder="Nominal Spp Siswa" value="{{ old('nominal') }}"
                                        name="nominal">
                                    ...
                                    <select class="form-control @error('tanggal') is-invalid @enderror" id="tanggal"
                                        ...
                                        @foreach (range(1, $daysInThisMonth) as $tanggal)
                                            <option value="{{ $tanggal }}"
                                                @if (old('tanggal') == $tanggal || date('d') == $tanggal) selected @endif>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-control @error('bulan') is-invalid @enderror" id="bulan"
                                        ...
                                        @foreach ($months as $bulan)
                                            <option value="{{ $bulan }}"
                                                @if (old('bulan') == $bulan) selected @endif>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-control @error('tahun') is-invalid @enderror" id="tahun"
                                        ...
                                        @for ($year = (int) date('Y'); 1900 <= $year; $year--)
                                            <option value="{{ $year }}"
                                                @if ($year == (int) date('Y') || old('year') == $year) selected @endif>{{ $year }}
                                            </option>
                                        @endfor
                                    </select>
